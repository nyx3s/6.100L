...
def filter(pixels_list, color):
    ...
    for pixel in pixels_list:
        m = matrix_multiply(trans_matrix, pixel)
        #print(pixel,m)
...
def extract_end_bits(num_end_bits, pixel):
    ...
    def ex_bin_int(num, strbiny):
        ...
        #print("after slice ",s,strbiny,num)
        #input()
    ...
    if type(pixel) == int:
        ...
        strbin = bin(pixel)[2:].zfill(8)
        a = ex_bin_int(num_end_bits, strbin)
        #print(a,strbin,num_end_bits)
        #input()
        return a
        ...
        for p in pixel:
            #print(p)
            re += (ex_bin_int(num_end_bits, bin(p)[2:].zfill(8)),)
        return re
    """
    # new version is much faster
...
def reveal_bw_image(filename):
    ...
    new_pixels = []
    for p in pixels:
        ne_p = extract_end_bits(1, p)
        new_pixels.append(ne_p * 255)

    re = pix_to_img(new_pixels, im.size, im.mode)
    return re
...
def reveal_color_image(filename):
    ...
    for p in pixels:
        ...
        rgb_new = ()
        for color in rgb:
            rgb_new += (rescale(color),)

        new_pixels.append(rgb_new)
...
def main():
    ...
    im2.save("flated_image.png")
    draw_kerb("flated_image.png", "time_bw")
    ...
    draw_kerb("unhidden2.bmp", "nyxx")
    draw_kerb("unhidden1.bmp", "nyxx")
