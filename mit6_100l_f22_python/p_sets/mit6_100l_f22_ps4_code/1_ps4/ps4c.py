...
def decrypt_message_try_pads(ciphertext, pads):
    ...
    for pad in pads:
        """this line cost me 4 days from my life bc i coldnot understand the doc
           bc the doc is boor the pram ciphertext was an object and i thohut is a string
           so i creat an object repsntsion string form the cipher i hope i learn from it
           and my futher self is beeter and more geny than i was today"""
        #crr_en_mesg = ps4b.EncryptedMessage(ciphertext)
        
        string = ciphertext.decrypt_message(pad).get_text()
        words = string.split()
    ...
    return ciphertext.decrypt_message(my_pad)
...
def decode_story():
    ...
    '''
    cipher = ps4b.EncryptedMessage(get_story_string())
    pads = get_story_pads()
    return decrypt_message_try_pads(cipher, pads).get_text()
