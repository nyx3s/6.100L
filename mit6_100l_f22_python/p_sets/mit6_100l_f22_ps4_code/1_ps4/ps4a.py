...
heap1 = Node(7,Node(3,Node(2),Node(1)),Node(6,Node(5),Node(4)))
...
def find_tree_height(tree):
    ...
    #print(tree)
    if tree != None:
        return 1+max(find_tree_height(tree.get_left_child()),\
...
def is_heap(tree, compare_func):
    ...
    if tree.get_left_child() == None\
        ...
        return compare_func(tree.get_right_child().get_value(),tree.get_value())\
                and is_heap(tree.get_right_child(),compare_func)
    elif tree.get_right_child() == None:
        return compare_func(tree.get_left_child().get_value(),tree.get_value())\
...
if __name__ == '__main__':
    ...
    print(is_heap(tr1, min_heap))
    print(is_heap(tr2, min_heap))
