...
def new_dict(input_dict):
    """
    # in key word take copy in primtive typse
    ...
    for val in input_dict.values():
        ...
        input()
    return input_dict"""
    # in this example we accses the data and change it
...
def check_same_values(input_dict):
    ...
    seen = list(input_dict.values())
    (i,j) = (0,len(seen)-1)
    #print(i,j)
    while i <= j:
        #print(f"{seen[i]}  {seen[j]}")
        #:input()
        if seen[i] != seen[j]:
            return False
        i +=1
...
def dict_to_sorted_list(input_dict):
    a = [[k,v] for k,v in input_dict.items()]
    a.sort()
    a.reverse()
    return a
