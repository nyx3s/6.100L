...
def fib_list(n,l):
    if n >= len(l) - 1:
        l.append(0)
        l.append(0)
    if l[n] in l and l[n] != 0:
        return l[n]
    
    l[n] = fib_list(n-1,l) + fib_list(n-2, l)
    return l[n]
...
def score_count_n(x,d):
    ...
    if x in d:
        ...
        # and make a score of x-2 then add 2
        # and make a score of x-3 then add 3
    d[x] = score_count_n(x-1,d)+score_count_n(x-2,d)+score_count_n(x-3,d)
    return d[x]

d = {1:1, 2:2, 3:3}
#print(score_count_n(4,d))  # prints 6
#print(score_count_n(6,d))  # prints 20
#print(score_count_n(13,d))  # prints 1431
#print(score_count(32))
...
def total_len_recur(L):
    ...
    return crrLen + total_len_recur(L[1:])
...
test = [[1,2],3,[4],5,6,7]
#print(in_list_of_lists_mod(test, 4))  # prints True
test = [[1,2],[3,4,5],6,7]
...
#print(g([1,1,1,2,3,1,1], 1)) # should print 4
...
#print(h([1,2,[3],1,[[1],[2,[1,[2,1]]]]], 1))        # should print 5
...
# the solotion from the lec

def flattenn(L):
    result = []
    for i in L:
        if type(i) == list:
            result.extend(flattenn(i))
        else:
            result.append(i)
    return result
