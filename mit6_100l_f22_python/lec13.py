...
def pairwise_div(Lnum, Ldenom):
    ...
    assert len(Lnum) == len(Ldenom) , "shold be same length"
    assert len(Lnum) or len(Ldenom) != 0 , "not empty"
    try:
        return [Lnum[i]/Ldenom[i] for i in range(len(Lnum))]
    except:
        raise ValueError("the List of denomneter is contains zero value")
